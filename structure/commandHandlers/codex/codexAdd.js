...
export default async function (interaction, pool) {
    ...
    try {
        ...
        if (categoryResult.rows.length === 0) {
            return interaction.reply({ content: i18n.t("errors.categoryNotFound", { lng: lang }), flags: MessageFlags.Ephemeral });
        }
        ...
        await interaction.reply({ content: i18n.t("info.codexEntryAdded", { lng: lang }), flags: MessageFlags.Ephemeral });

    } catch (error) {
        console.error("Ошибка при добавлении в кодекс:", error);
        await interaction.reply({ content: i18n.t("errors.codexEntryAddError", { lng: lang }), flags: MessageFlags.Ephemeral });
    }
}